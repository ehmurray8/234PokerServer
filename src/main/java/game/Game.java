...
public class Game {
    ...
    private void createHand() {
        ...
        switch (currentGameType) {
            case PINEAPPLE:
                currentHand =
                    new PineappleHand(rules.getSmallBlind(), rules.getBigBlind(), rules.getAnte(),
                        playersInHand);
                break;
            case OMAHA:
                currentHand =
                    new OmahaHand(rules.getSmallBlind(), rules.getBigBlind(), rules.getAnte(),
                        playersInHand);
                break;
            case TEST:
                currentHand =
                    new TestHand(rules.getSmallBlind(), rules.getBigBlind(), rules.getAnte(),
                        playersInHand);
                break;
            default:
                currentHand =
    ...
    private HashMap<UUID, RegularUpdate> createUpdates() {
        ...
        for (var player : players) {
            if (player != null) {
                var update = new RegularUpdate(
                    currentGameType, player,
                    Arrays.asList(players).indexOf(player),
                    players,
                    playersInHand,
                    currentHand.getCommunityCards(),
                    currentAction,
                    actionLimit
                );
